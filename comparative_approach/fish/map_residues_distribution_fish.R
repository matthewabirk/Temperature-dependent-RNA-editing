...
syt_align = seqinr::read.fasta('fish/teleost.fa', seqtype = 'AA', as.string = TRUE)

library(stringr)

syt_align = data.frame(species = unname(gsub('.*\\[(.*)\\]', '\\1', sapply(syt_align, function(i) attr(i, 'Annot')))),
											 msa = as.character(unlist(syt_align)))
syt_align$residue = str_sub(syt_align$msa, start = 314, 314)

syt_align = syt_align[!duplicated(syt_align$species), ]

tmp = str_sub(syt_align$msa, start = 309, 319)



ggseqlogo::ggseqlogo(tmp, method = 'prob') +
	annotate('rect', xmin = 5.5, xmax = 6.5, ymin = -0.1, ymax = 1.1, alpha = 0.1, col = 'black', fill = 'yellow') +
	ggtitle('Alignment of teleost matches to Ocbimv22021175m')

ggsave('XXX.pdf', width = 8, height = 3)
